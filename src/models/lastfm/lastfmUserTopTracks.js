...
async function createTable (lastfmUserTracks_db) {
    const CREATE_TABLE = `CREATE TABLE IF NOT EXISTS lastfmUserTopTracks(
...
export async function insertIntoTable (currPage, lastfmUserTracks_db, artist, playcount, song) {

    const INSERT_INTO_TABLE = `INSERT INTO lastfmUserTopTracks ( artist, playcount, song)
                                VALUES (${JSON.stringify(artist)}, 
                                        ${playcount}, 
                                        ${JSON.stringify(song)}
                                        )`;
